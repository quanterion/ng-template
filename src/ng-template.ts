...
function createEvaluator(context: any): Evaluator {
  let entries = [];
      ...
      if (parser && parser.length === 4) {
        ...
        if (array && array.length > 0) {
          ...
          let promises = elements.map(
            (e, index) => compileTemplate(e, {...context, [varName]: array[index]})
          );
...
export async function compileTemplate(element: Element, context?: any) {
